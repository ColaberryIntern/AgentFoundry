...
// ---------------------------------------------------------------------------
// Executor — picks up simulation_passed actions and executes them
// ---------------------------------------------------------------------------
...
async function executeCreateUseCase(
  params: Record<string, unknown>,
): Promise<Record<string, unknown>> {
  const industryCode = params.industryCode as string;
  ...
  const outcomeStatement = `Automate compliance monitoring coverage for ${ind.title} (NAICS ${ind.code})`;

  await execSql(
    `
    INSERT INTO use_cases
      (id, outcome_statement, measurable_kpi, industry_scope, regulatory_scope, urgency_score, capital_dependency_score, monetization_type, status, version, "createdAt", "updatedAt")
    VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'active', 1, NOW(), NOW())
    `,
    [
      useCaseId,
      outcomeStatement,
      `Coverage gap reduction for ${ind.title}`,
      JSON.stringify([industryCode]),
      JSON.stringify([]),
      Math.min(spiScore / 100, 1.0),
      0.3,
      monetizationType,
    ],
  );
  ...
  await execSql(
    ...
    INSERT INTO ontology_relationships
      (id, subject_type, subject_id, relationship_type, object_type, object_id, weight, version, "createdAt", "updatedAt")
    VALUES ($1, 'use_case', $2, 'APPLIES_TO', 'naics_industry', $3, 1.0, 1, NOW(), NOW())
    ON CONFLICT DO NOTHING
    `,
    [crypto.randomUUID(), useCaseId, industryCode],
  );

  await execSql(
    `
    INSERT INTO registry_audit_log (id, actor, action, entity_type, entity_id, changes, reason, "createdAt")
    VALUES ($1, 'orchestrator', 'create', 'use_case', $2, $3, 'Auto-generated by Executive Orchestrator', NOW())
    `,
    [
      crypto.randomUUID(),
      useCaseId,
      JSON.stringify({ outcomeStatement, industryCode, monetizationType }),
    ],
  );

  return { useCaseId, outcomeStatement, industryCode, monetizationType };
}
    ...
    try {
      ...
      switch (row.action_type) {
        case 'create_use_case':
          result = await executeCreateUseCase(params);
          break;
        case 'create_variant':
